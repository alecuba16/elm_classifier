...
ranking_by_id_walm <- function(table_park_dic = "1_cast_park_table_dic", park, date1 = '1990-01-01', date2 = Sys.time(),db_config) {
    iam='ranking_by_id_walm'
    #Dependencia basica
    ...
    dep<-dependencyLoader(c('plyr','dplyr','reshape2','functions_common/db_query.R'))
    ...
    unique(x$alarm)
    mx <- reshape2::dcast(x, id_walm ~ ld_id, sum, value.var = "alarm")
